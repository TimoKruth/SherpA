...
func NewWithOptions(st store.Store, cs content.ContentStore, adminToken string, github registryauth.GitHubClient, options Options) http.Handler {
	...
	mux.HandleFunc("PUT /v1/me/follows/{owner}/{name}/seen", s.handleMarkSeen)
	mux.HandleFunc("PUT /v1/me/trials/{owner}/{name}/{version}", s.handlePutTrial)
	mux.HandleFunc("GET /v1/search", s.handleSearch)
	mux.HandleFunc("GET /v1/users/{handle}", s.handleUser)
